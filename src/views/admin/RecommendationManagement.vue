<template>
  <div class="recommendation-management">
    <el-page-header @back="handleBack">
      <template #content>
        <span class="page-title">推荐管理</span>
      </template>
    </el-page-header>
        ...
        <div class="table-wrapper">
          <el-table :data="tableData" v-loading="loading" border stripe>
            ...
            <el-table-column label="推荐理由" min-width="260" show-overflow-tooltip>
              <template #default="{ row }">
                <div class="reason-tags">
                  <el-tag v-for="reason in parseReasons(row.reasons)" :key="`${row.recommendation_id}-${reason}`" size="small" effect="plain">
                    {{ reason }}
                  </el-tag>
                </div>
              </template>
            ...
            <el-table-column label="操作" width="130" fixed="right" align="center">
              <template #default="{ row }">
                <el-button type="primary" link :loading="refreshRowLoading === row.student_id" @click="handleRefreshSingle(row.student_id)">
                  刷新该用户
                </el-button>
              </template>
            </el-table-column>
...
const handleRefreshByStudent = async () => {
  ...
  try {
    await recommendationsApi.refresh({
      student_id: refreshForm.student_id,
